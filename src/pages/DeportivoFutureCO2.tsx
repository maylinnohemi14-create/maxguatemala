...
const DeportivoFutureCO = () => {
  ...
  const mainQty = Math.min(selectedQty, 3);
  const bonusQty = selectedQty === 5 ? 2 : 0;

  const extraNote = useMemo(() => {
    const parts: string[] = [];
    parts.push(`CONJUNTO ULTRA DRY x${mainQty}`);
    ...
    return parts.join(" | ");
  }, [selectedQty, selectedColors, setSizes, addShirt, shirtSize, mainQty, bonusQty]);
            ...
            <div className="mb-6">
              <div className="flex items-center justify-between mb-3">
                <div className="text-xs uppercase tracking-widest text-white/60 font-bold">
                  ✦ Elige {mainQty === 1 ? "tu color" : `tus ${mainQty} colores`}{bonusQty > 0 ? " (Deportivo)" : ""}
                </div>
                <div className="text-[11px] text-white/50">
                  {selectedColors.length}/{mainQty}
                </div>
              </div>
