...
import { JOGGER_KITS_CO_PRODUCTS } from "./JoggerKits";
...
const JoggerKitsProductPage = () => {
  ...
  const product = JOGGER_KITS_CO_PRODUCTS.find((p) => p.slug === slug);

  const suggestions = useMemo(() => {
    if (!product) return [];
    return JOGGER_KITS_CO_PRODUCTS.filter((p) => p.slug !== slug).slice(0, 4);
  }, [slug, product]);
  ...
  const handleBuyClick = () => {
    trackTikTokConversion(
      "AddToCart",
      { contents: [{ content_id: product.id, content_type: "product", content_name: product.name, quantity: qty, price: totalPrice }], value: totalPrice, currency: "COP" },
      undefined
    );
    trackFacebookConversion(
      "AddToCart",
      { content_ids: [product.id], content_type: "product", value: totalPrice, currency: "COP" },
      undefined
    );
    setShowUpsell(true);
  };
      ...
      <header className="relative z-10 border-b border-white/10 backdrop-blur-md bg-black/40">
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            >
              <svg viewBox="0 0 24 12" className="w-6 h-3" fill="white">
                <path d="M24 1.5c-3 5-8.5 9-15 9-3.5 0-6-1-7-2.5C2.5 9 4 8.5 6 8c4-1 9-3 13-5.5 1.5-1 3-2 5-2.5z" />
              </svg>
            </div>
            <div>
              <div className="text-[10px] uppercase tracking-[0.3em] text-white/50">Colección Vintage</div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
          ...
          <div className="text-white">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full mb-4 border backdrop-blur-md"
              ...
              <span className="text-[11px] font-bold tracking-widest" style={{ color: NIKE_ORANGE }}>COLECCIÓN VINTAGE 2026</span>
            </div>
            ...
            <p className="text-white/60 text-sm sm:text-base mb-5">
              Pantalón deportivo vintage original. Pieza única con acabado premium. Envío gratis a toda Colombia.
            </p>
            ...
            <div className="flex items-center gap-2 mb-4">
              ...
              <span className="text-xs text-white/50">(4.9) · 89 ventas</span>
            </div>
            ...
            <div className="mb-6 p-4 rounded-2xl border border-white/10 bg-white/5 backdrop-blur">
              <div className="flex items-baseline gap-3 mb-1">
                <span className="text-4xl font-black" style={{ color: NIKE_ORANGE }}>{cop(product.price)}</span>
                <span className="text-lg text-white/30 line-through">{cop(product.originalPrice)}</span>
            ...
            {/* Quantity */}
            <div className="mb-6">
              <div className="text-xs uppercase tracking-widest text-white/50 mb-3 font-bold">✦ Cantidad</div>
              <div className="flex items-center gap-3">
                <button onClick={() => setQty(Math.max(1, qty - 1))}
                  ...
                  style={{ borderColor: `${NIKE_ORANGE}66`, background: `${NIKE_ORANGE}15`, color: NIKE_ORANGE }}>−</button>
                <div className="w-14 h-10 rounded-xl border-2 flex items-center justify-center text-lg font-black"
                  style={{ borderColor: NIKE_ORANGE, background: `linear-gradient(135deg, ${NIKE_ORANGE}22, ${NIKE_RED}11)`, color: NIKE_ORANGE, boxShadow: `0 0 16px ${NIKE_ORANGE}33` }}>
                  {qty}
                </div>
                <button onClick={() => setQty(qty + 1)}
                  className="w-10 h-10 rounded-xl border-2 flex items-center justify-center text-lg font-black transition-all hover:scale-105"
                  style={{ borderColor: `${NIKE_ORANGE}66`, background: `${NIKE_ORANGE}15`, color: NIKE_ORANGE }}>+</button>
                <span className="text-sm text-white/40 ml-2">{cop(totalPrice)}</span>
              </div>
            </div>
            ...
            <div className="mb-6">
              <div className="flex items-center justify-between mb-3">
                <div className="text-xs uppercase tracking-widest text-white/50 font-bold">✦ Talla</div>
                  ...
                  return (
                    <button key={s} onClick={() => setSelectedSize(s)}
                      ...
                      style={{
                        ...
                        background: active ? `linear-gradient(135deg, ${NIKE_ORANGE}, ${NIKE_RED})` : "transparent",
                        color: "#fff",
            ...
            <Button onClick={handleBuyClick} size="lg"
              ...
              <ShoppingBag className="w-5 h-5 mr-2" /> COMPRAR AHORA — {cop(totalPrice)}
            </Button>
      ...
      <section className="relative z-10 max-w-6xl mx-auto px-4 pb-16">
        <div className="text-center mb-8">
          ...
          <h2 className="text-2xl sm:text-3xl font-black text-white">
            Más Estilos <span style={{ color: NIKE_ORANGE }}>Exclusivos</span>
          </h2>
        </div>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4">
          {suggestions.map((p) => (
            <Link key={p.slug} to={`/joggerkits/${p.slug}`}
              ...
              <div className="p-3">
                ...
                <div className="flex items-baseline gap-2">
                  <span className="text-base font-black" style={{ color: NIKE_ORANGE }}>{cop(p.price)}</span>
                  <span className="text-xs text-white/30 line-through">{cop(p.originalPrice)}</span>
                </div>
              </div>
            </Link>
          ))}
...
export default JoggerKitsProductPage;
