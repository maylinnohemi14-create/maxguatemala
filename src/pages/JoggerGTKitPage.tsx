...
import { Check, Shield, Truck, Sparkles, Star, ChevronLeft, ShoppingBag, CreditCard, Ruler, Package } from "lucide-react";
import { Dialog, DialogContent, DialogHeader, DialogTitle } from "@/components/ui/dialog";
import { JOGGER_GT_KITS } from "./JoggerGT";
...
const JoggerGTKitPage = () => {
  ...
  const kit = JOGGER_GT_KITS.find((k) => k.slug === slug);

  const otherKits = useMemo(() => {
    if (!kit) return [];
    return JOGGER_GT_KITS.filter((k) => k.slug !== slug).slice(0, 4);
  }, [slug, kit]);
  ...
  const handleBuyClick = () => {
    trackTikTokConversion(
      "AddToCart",
      { contents: [{ content_id: kit.id, content_type: "product", content_name: productDisplayName, quantity: 1, price: totalPrice }], value: totalPrice, currency: "GTQ" },
      undefined
    );
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
          ...
          <div className="text-white">
            ...
            <p className="text-white/60 text-sm sm:text-base mb-5">
              3 pantalones deportivos vintage en un solo kit. Ahorra comprando el combo completo. Envío gratis a toda Guatemala.
            </p>
            ...
            <div className="mb-6 p-4 rounded-2xl border border-white/10 bg-white/5 backdrop-blur">
              <div className="flex items-baseline gap-3 mb-1">
                <span className="text-4xl font-black" style={{ color: NIKE_ORANGE }}>Q{kit.price}</span>
                <span className="text-lg text-white/30 line-through">Q{kit.originalPrice}</span>
                <span className="px-2 py-0.5 rounded-full text-[11px] font-black"
                  style={{ background: `${NIKE_ORANGE}33`, color: NIKE_ORANGE }}>
                  AHORRA Q{kit.originalPrice - kit.price}
                </span>
              </div>
            ...
            <Button onClick={handleBuyClick} size="lg"
              ...
              <ShoppingBag className="w-5 h-5 mr-2" /> COMPRAR KIT 3en1 — Q{totalPrice}
            </Button>
      ...
      {otherKits.length > 0 && (
        <section className="relative z-10 max-w-6xl mx-auto px-4 pb-16">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4">
            {otherKits.map((k) => (
              <Link key={k.slug} to={`/joggergt/kit/${k.slug}`}
                ...
                <div className="p-3">
                  ...
                  <div className="flex items-baseline gap-2">
                    <span className="text-base font-black" style={{ color: NIKE_ORANGE }}>Q{k.price}</span>
                    <span className="text-xs text-white/30 line-through">Q{k.originalPrice}</span>
                  </div>
                </div>
              </Link>
            ))}
...
export default JoggerGTKitPage;
