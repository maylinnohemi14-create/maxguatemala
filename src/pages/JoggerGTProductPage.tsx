...
const JoggerGTProductPage = () => {
  ...
  const product = JOGGER_GT_PRODUCTS.find((p) => p.slug === slug);

  const suggestions = useMemo(() => {
    if (!product) return [];
    return JOGGER_GT_PRODUCTS.filter((p) => p.slug !== slug).slice(0, 4);
  }, [slug, product]);
  ...
  if (!product) {
    return <Navigate to="/joggergt" replace />;
  }
  ...
  const handleBuyClick = () => {
    trackTikTokConversion(
      "AddToCart",
      { contents: [{ content_id: product.id, content_type: "product", content_name: product.name, quantity: qty, price: totalPrice }], value: totalPrice, currency: "GTQ" },
      undefined
    );
    ...
    >
      ...
      <header className="relative z-10 border-b border-white/10 backdrop-blur-md bg-black/40">
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Link to="/joggergt" className="flex items-center gap-2 text-white/60 hover:text-white transition-colors">
            ...
            <div>
              ...
              <div className="text-base sm:text-lg font-black tracking-tight text-white">
                TRACKPANTS <span style={{ color: NIKE_ORANGE }}>·</span> GT
              </div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
          ...
          <div className="text-white">
            ...
            <p className="text-white/60 text-sm sm:text-base mb-5">
              Pantalón deportivo vintage original. Pieza única con acabado premium. Envío gratis a toda Guatemala.
            </p>
            ...
            <div className="mb-6 p-4 rounded-2xl border border-white/10 bg-white/5 backdrop-blur">
              <div className="flex items-baseline gap-3 mb-1">
                <span className="text-4xl font-black" style={{ color: NIKE_ORANGE }}>Q{product.price}</span>
            ...
            <div className="mb-6">
              ...
              <div className="flex items-center gap-3">
                ...
                <button onClick={() => setQty(qty + 1)}
                  className="w-10 h-10 rounded-xl border-2 flex items-center justify-center text-lg font-black transition-all hover:scale-105"
                  style={{ borderColor: `${NIKE_ORANGE}66`, background: `${NIKE_ORANGE}15`, color: NIKE_ORANGE }}>+</button>
                <span className="text-sm text-white/40 ml-2">Q{totalPrice}</span>
              </div>
            </div>
            ...
            <Button onClick={handleBuyClick} size="lg"
              ...
              <ShoppingBag className="w-5 h-5 mr-2" /> COMPRAR AHORA — Q{totalPrice}
            </Button>
      ...
      <section className="relative z-10 max-w-6xl mx-auto px-4 pb-16">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4">
          {suggestions.map((p) => (
            <Link key={p.slug} to={`/joggergt/${p.slug}`}
              ...
              <div className="p-3">
                ...
                <div className="flex items-baseline gap-2">
                  <span className="text-base font-black" style={{ color: NIKE_ORANGE }}>Q{p.price}</span>
...
export default JoggerGTProductPage;
